...
export async function runExistingThread(context: INodeContext): Promise<INodeExecutionData> {
  const { openai, functionThis, items, i } = context;
  
  const threadId = functionThis.getNodeParameter('existingThreadId', i) as string;
  ...
  if (messageInputMethod === 'singleMessage') {
    // 단일 메시지 처리
    const additionalMessage = functionThis.getNodeParameter('threadAdditionalMessage', i) as string;
  ...
  if (!waitForCompletion) {
    return {
      json: { thread_id: threadId, run: { ...run as any } }
    };
  } else {
    // 완료까지 대기
    ...
    if (simplifyOutput) {
      ...
      responseData = { thread_id: threadId, run: { ...runStatus as any } };
    }
